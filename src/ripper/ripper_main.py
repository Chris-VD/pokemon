...
- Starmie (/wiki/Starmie_(Pok%C3%A9mon)) -> {"Number": "#0121"}
'''
...
from pokemon_check import check
...
def import_poke(pokemon_link: str) -> dict:
    ...
    for tr in PokemonAttackList_Raw:
        ...
        try:
            AttackStats = {"N": AttackName, "Unlock": AttackLevelUnlock, "C": AttackCategory, "T": AttackType, "P": AttackPower, "ACC": AttackAccuracy, "PP": AttackPowerPoints}
            AttackList.append(AttackStats)
        except:
            continue

    # Once absolutely everything is done, we just export the result to a json with all the info we want.
    PokemonFull = {"Number": PokemonNumber,"N": PokemonName, "T": PokemonType, "ATK": PokemonATK, "DEF": PokemonDEF, "SPD": PokemonSPD, "HP": PokemonHP, "Moves": AttackList}
    PokemonFullList.append(PokemonFull)
    print(PokemonNumber, PokemonName)
...
json.dump(PokemonFullList, open("pokemons_raw.json", "w"))
...
# Checks if the list of pokemon if full or if any pokemon is missing
check()
